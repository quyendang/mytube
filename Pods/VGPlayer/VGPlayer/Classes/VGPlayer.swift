...
open class VGPlayer: NSObject {
    var currentVideo: [String: String]?
    ...
    open var backgroundMode : VGPlayerBackgroundMode = .proceed
...
extension VGPlayer {
    ...
    open func replaceVideoData(_ video: [String: String]){
...
extension VGPlayer {
    ...
    func updateLockedScreenMusic() {
        if self.currentVideo == nil {return}
        if let urlString = self.currentVideo!["image"] {
            let url = URL(string: urlString)
            getDataFromUrl(url: url!) { (data, error) in
                DispatchQueue.main.async {
                    let center = MPNowPlayingInfoCenter.default()
                    var info = [String: Any]()
                    //info[MPMediaItemPropertyAlbumTitle] = self.currentVideo!["channelTitle"]!
                    if let channelTitle = self.currentVideo!["channelTitle"] {
                        info[MPMediaItemPropertyArtist] = channelTitle
                    }
    
                    if let title = self.currentVideo!["title"] {
                        info[MPMediaItemPropertyTitle] = title
                    }
                    
                    if error == nil {
                        if data != nil {
                            info[MPMediaItemPropertyArtwork] = MPMediaItemArtwork(image: UIImage(data: data!)!)
                        }
                    } else{
                        info[MPMediaItemPropertyArtwork] = MPMediaItemArtwork(image:UIImage(named: "video_placeholder")!)
                    }
                    if self.player!.currentItem != nil{
                        info[MPMediaItemPropertyPlaybackDuration] = NSNumber(floatLiteral: CMTimeGetSeconds(self.player!.currentItem!.duration))
                        info[MPNowPlayingInfoPropertyElapsedPlaybackTime] = NSNumber(floatLiteral: CMTimeGetSeconds(self.player!.currentItem!.currentTime()))
                    }
